...
export const CampaignModal = ({ isOpen, onClose }: ICampaignModal) => {
  ...
  useEffect(() => {
    if (updateCampaign.isSuccess) {
      setTimeout(() => {
        setIsSaving(false);
        setIsSaved(true);
        context?.setCampaign(updateCampaign?.data as Campaign);
      }, 300);
    }
  ...
  useEffect(() => {
    if (context?.campaign?.updatedAt) {
      setUpdatedAt(
        format(
          new Date(context?.campaign?.updatedAt),
          "dd/MM/yyyy à HH:mm:ss"
        ).toString()
      );
    }
  }, [isOpen, context?.campaign?.updatedAt, updateCampaign.isSuccess]);
  ...
  const canGoNextStep = (step: number, camp?: Partial<Campaign>) => {
    if (context?.campaign?.status === "sent") {
      return true;
    }
    switch (step) {
      case 0:
          ...
          <Box gap={1}>
            ...
            {!isSaving && isSaved && (
              <Flex alignItems={"center"} gap={2}>
                ...
                <Text fontSize={13}>Sauvegardée</Text>
              </Flex>
            )}
            {isSending && (
              <Flex alignItems={"center"} gap={2}>
            ...
            <Divider my={2} w={"full"} />
            {context?.campaign?.creatorId && (
              <Box>
                <Text fontSize={13} color="gray.500">
                  Créée par{" "}
                  <Text as="span">
                    <Text as="span" fontWeight="bold" color="gray.600">
                      {context?.campaign?.user?.name}
                    </Text>
                    , le{" "}
                    {format(
                      new Date(context?.campaign?.createdAt ?? ""),
                      "dd/MM/yyyy à HH:mm"
                    )}
                  </Text>
                </Text>
